(function() {
  ...
  **/
  ...
  function getColorScales(data) {
    var norm = document.querySelector('#relationship-normalize').value;
    state.norm = norm;

    /*
      Each observation in data represents the cooccurrence count
      of level1, level2 within the graph. Certain of those levels
      are simply more likely to occur though, so divide each value
      by either level1's frequency in the population or level2's
      frequency in the population, according to whether user has
      selected to normalize by rows or columns
    */
  ...
  **/
  ...
  function getDatafilePath() {
    var x = document.getElementById('relationship-x').value;
    var y = document.getElementById('relationship-y').value;
    return chart.data.dir + 'edge_cooccurrences/' + x + '_' + y + '.json';
  } 
  ...
  **/
  ...
  // cache the raw_metadata_frequencies for normalization
  d3.json(chart.data.dir + 'normalized_metadata_frequencies.json', function(json) {
    chart.data.normalizedFrequencies = json;
    d3.json(getDatafilePath(), initializeChart)
  })

  /**
  * Add public api that's called when users change select value
  **/

  // add event listeners
  var selectsContainer = document.querySelector('.relationship-chart-selects');
